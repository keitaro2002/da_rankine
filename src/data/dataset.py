...
class RankineData():
        ...
        if self.number_of_prior in [1, 2, 3]:
            ...
            for key in self.params_true.keys():
                if key == 'U' or key == 'R':
                    ...
                    params_prior[key] = np.random.normal(self.params_true[key], std_dev, self.ensemble_size)
        
        elif self.number_of_prior == 4:
            params_prior = {}
            for key in self.params_true.keys():
                if key == 'U' or key == 'R':
                    params_prior[key] = np.random.normal(self.params_true[key], 3, self.ensemble_size)
                else:
    ...
    def calculate_windspeeds_prior(self):
        windspeeds_prior = {'u_x': np.zeros((self.ensemble_size, 91, 91)),
